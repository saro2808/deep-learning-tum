...
class Encoder(nn.Module):
    ...
    def __init__(self, hparams, input_size=28 * 28, latent_dim=20):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(self.hparams['input_dim'], self.hparams['enc_hidden_dim']),
            nn.BatchNorm1d(self.hparams['enc_hidden_dim']),
            nn.ReLU(),
            nn.Linear(self.hparams['enc_hidden_dim'], latent_dim),
...
class Decoder(nn.Module):
    ...
    def __init__(self, hparams, latent_dim=20, output_size=28 * 28):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, self.hparams['enc_hidden_dim']),
            nn.BatchNorm1d(self.hparams['enc_hidden_dim']),
            nn.ReLU(),
            nn.Linear(self.hparams['enc_hidden_dim'], self.hparams['input_dim'])
        )
...
class Autoencoder(nn.Module):
    ...
    def training_step(self, batch, loss_func):
        ...
        ########################################################################
        # TODO:                                                                #
        # Complete the training step, similar to the way it is shown in        #
...
class Classifier(nn.Module):
    ...
    def __init__(self, hparams, encoder):
        ...
        self.model = nn.Sequential(
            nn.Linear(encoder.latent_dim, self.hparams['cls_hidden_dim']),
            # # nn.BatchNorm1d(self.hparams['cls_hidden_dim']),
            nn.Dropout(0.1),
            nn.ReLU(),
            nn.Linear(self.hparams['cls_hidden_dim'], self.hparams['cls_hidden_dim']),
            nn.Dropout(0.1),
            nn.ReLU(),
            nn.Linear(self.hparams['cls_hidden_dim'], self.hparams['cls_hidden_dim']),
            nn.Dropout(0.1),
            nn.ReLU(),
            nn.Linear(self.hparams['cls_hidden_dim'], self.hparams['num_classes'])
