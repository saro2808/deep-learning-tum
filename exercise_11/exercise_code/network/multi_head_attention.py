...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        q = self.weights_q(q).reshape(batch_size, -1, self.n_heads, self.d_k).transpose(1, 2)
        k = self.weights_k(k).reshape(batch_size, -1, self.n_heads, self.d_k).transpose(1, 2)
        v = self.weights_v(v).reshape(batch_size, -1, self.n_heads, self.d_v).transpose(1, 2)

        outputs = self.attention(q, k, v).transpose(1, 2)
        outputs = outputs.reshape(batch_size, sequence_length_queries, -1)
